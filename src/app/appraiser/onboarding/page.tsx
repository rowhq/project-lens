...
// Texas cities loaded dynamically from API
...
export default function AppraiserOnboardingPage() {
  ...
  const { data: serviceAreas } = trpc.appraiser.serviceAreas.getCities.useQuery(
    { state: selectedState },
    { staleTime: 5 * 60 * 1000 }, // Cache for 5 minutes
  );
  const { data: supportedStates } =
    trpc.appraiser.serviceAreas.getStates.useQuery();

  // Use API cities or fallback
  const availableCities = serviceAreas?.cities || fallbackCities;
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleInputChange("homeBaseAddress", e.target.value)
                  }
              ...
              <div>
                ...
                {/* State Selector */}
                {supportedStates && supportedStates.length > 1 && (
                  <div className="mb-3">
                    ...
                    <select
                      ...
                      className="w-full px-3 py-2 border border-[var(--border)] rounded-lg focus:outline-none focus:ring-2 focus:ring-[var(--primary)] bg-[var(--card)] text-[var(--foreground)]"
                    >
                      {supportedStates
                        .filter((s) => s.active)
                        .map((state) => (
                ...
                <p className="text-xs text-[var(--muted-foreground)] mb-3">
                  Select the {serviceAreas?.stateName || "Texas"} cities you can
                  service
                </p>
                ...
                <p className="text-xs text-[var(--muted-foreground)] mt-2">
                  {formData.servicedCities.length} of {availableCities.length}{" "}
                  cities selected
                </p>
              </div>
            </div>
            ...
            <div className="space-y-6">
              ...
              <div className="p-6 bg-[var(--secondary)] rounded-lg border border-[var(--border)] text-center">
                ...
                <p className="text-sm text-[var(--muted-foreground)] mb-4">
                  We use Stripe Connect for secure, fast payouts directly to
                  your bank account. Your banking information is never stored on
                  our servers.
                </p>
              ...
              <div className="p-4 bg-[var(--primary)]/20 rounded-lg border border-[var(--primary)]/30">
                ...
                <ul className="text-sm text-[var(--primary)]/80 space-y-1">
                  ...
                  <li>
                    2. We&apos;ll verify your license with the state board
                  </li>
                  <li>
                    3. Once approved, you&apos;ll receive job notifications
                  </li>
                </ul>
              </div>
            </div>
